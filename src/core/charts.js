...
var devicePixelBackingStoreRatio;
...
Chart.prototype.renderIndexLabels = function (targetCtx) {
  ...
  for (var i = 0; i < this._indexLabels.length; i++) {
    ...
    if (chartTypeLower.indexOf("stackedcolumn") !== -1) {
      if (indexLabel.dataPoint.y > 6) textBlock.render(true);
    } else if (chartTypeLower.indexOf("column") !== -1) {
      var dataPointsLength = this.data[0].dataPoints.length;
      var dataBarWidth = (plotArea.width - (marginX * 2 * dataPointsLength)) / this._indexLabels.length;

      if (Math.floor(indexLabel.dataPoint.y) !== 0) {
        if (dataBarWidth > 15 || i < dataPointsLength) {
          textBlock.render(true);
        } else {
          var prevLabel = this._indexLabels[i - dataPointsLength];
          var heightDifference = Math.abs(indexLabel.bounds.y1 - prevLabel.bounds.y1);

          if (heightDifference > 7) {
            textBlock.render(true);
          }
        }
      }

    } else if (chartTypeLower.indexOf("bar") !== -1) {
      var dataPointsLength = this.data[0].dataPoints.length;
      var dataBarHeight = (plotArea.width - (marginX * 2 * dataPointsLength)) / this._indexLabels.length;

      if (Math.floor(indexLabel.dataPoint.y) !== 0) {
        if (dataBarHeight > 15 || i < dataPointsLength) {
          textBlock.render(true);
        } else {
          var prevLabel = this._indexLabels[i - dataPointsLength];
          var widthDiffernce = Math.abs(indexLabel.bounds.x2 - prevLabel.bounds.x2);

          if (widthDiffernce > 10) {
            textBlock.render(true);
          }
        }
      }
